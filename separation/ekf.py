...
CHANGES FROM ORIGINAL:
  [FIX-1] Phase burn-in added to filter() and smooth().
  [FIX-2] EKF_PROCESS_NOISE raised in config.py.

[NEW] Stricter EKF acceptance gate in _apply_ekf() (pipeline.py):
      EKF output is used only if ALL THREE gates pass:
        (a) peak count post-EKF >= 70% of pre-EKF
        (b) CC(ekf_output, pre_ekf_input) >= 0.60
        (c) |median_RR_post - median_RR_pre| <= 15ms  [NEW in v5]

      Gate (c) is the critical addition. Gates (a) and (b) cannot detect
      a 20-40ms phase shift because:
        - A phase-shifted signal has the same peak count (a passes)
        - Global CC of a 30ms-shifted 60s signal is ~0.997 (b passes)
      Gate (c) directly checks whether EKF has shifted the beat rhythm.
      A 15ms RR tolerance corresponds to ~1 BPM at 140 BPM fetal HR.

NOTE: _estimate_pqrst_from_beats (IMP-4) was trialled and removed.
      On morphologically inconsistent recordings (normal for low-SNR
      abdominal ECG), the beat-averaged template is noisy, producing
      bad PQRST alpha estimates. This caused EKF phase shifts on 9
      previously-good recordings. The fixed _FETAL_PQRST_DEFAULT
      (fetal-specific narrower QRS widths) is more stable.
"""
...
# Fetal-specific PQRST: narrower QRS (~40-60ms) vs adult (~80-100ms)
# Lower P and T amplitudes relative to R wave
_FETAL_PQRST_DEFAULT = np.array([
    [0.15,  0.08, -np.pi / 3],
    [-0.30, 0.04, -np.pi / 12],
    [1.50,  0.06,  0.0],
    [-0.30, 0.04,  np.pi / 12],
...
class FetalECGKalmanFilter:
    ...
    def __init__(self, fs=FS, fetal_hr_init=EKF_FETAL_HR_INIT,
                 pqrst_params=None):
        self.fs = fs
        self.dt = 1.0 / fs
        ...
        self.ecg_params = (pqrst_params if pqrst_params is not None
                           else _FETAL_PQRST_DEFAULT).copy()
    ...
    def update(self, state_pred, P_pred, observation):
    ...
    def _build_hr_updates(self, detected_peaks):
        hr_updates = {}
    ...
    def _phase_burnin(self, observed, detected_peaks, hr_updates):
    ...
    def filter(self, observed, detected_peaks=None):
        N = len(observed)
        filtered = np.zeros(N)
        state_log = []
    ...
    def smooth(self, observed, detected_peaks=None):
        N = len(observed)
